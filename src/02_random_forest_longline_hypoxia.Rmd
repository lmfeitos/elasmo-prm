...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, reproductive_mode, family, ac
  ) %>%
...
elasmo_avm <- prm_elasmo_subset %>%
  ...
  select(ventilation_method, median_depth, ac, max_size_cm, habitat, reproductive_mode, mortality_prop, family) %>%
  distinct() %>%
  drop_na()
...
```{r}
ggplot(avm_quant_test, aes(x = mortality_prop, y = .pred)) + # plot ln of real versus ln of predicted
  geom_point() +
  stat_poly_line() +
...
```{r}
ggplot(prm_train_predict, aes(x = mortality_prop, y = .pred)) + # plot ln of real versus ln of predicted
  geom_point() +
  stat_poly_line() +
  stat_poly_eq(use_label("eq")) +
  stat_poly_eq(label.y = 0.9) +

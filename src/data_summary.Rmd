...
prm_elasmo <- read_csv(here::here("data", "prm_dataset_updated.csv")) %>% 
  select(scientific_name, estimate, estimate_type, sample_size, gear_class, family, habitat_associated, reference)
...
gilman_join <- gilman_df %>% 
  ...
  select(scientific_name, estimate, sample_size, group, family, reference) %>% 
...
gilman_join_clean <- gilman_join %>% 
  distinct(scientific_name, estimate, sample_size, estimate_type)
```
...
gilman_join_summary <- gilman_join %>%
  distinct(scientific_name, group, estimate_type) %>% 

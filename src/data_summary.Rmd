...
sim_join <- read_csv(here("data", "sim_pct_correct_uncorrect.csv")) %>% 
  select(family, status, mean_abs_mort_reduction) %>% 
  distinct() %>% 
  pivot_wider(names_from = status,
              values_from = mean_abs_mort_reduction) %>% 
  clean_names()  %>% 
  group_by(family) %>% 
  summarise(diff = corrected - uncorrected) %>% 
  ungroup() %>% 
  mutate(mean_diff = mean(diff)) %>% 
  mutate(diff = round(diff * 100, 4))
```
...
```{r}
# call full_predictions dataset
full_pred <- read_csv(here("data", "full_predictions.csv")) %>% 
  select(scientific_name, max_size_cm)

# check
sim_results_subset <- sim_results_2 %>% 
  filter(t == 200 & mort_scenario %in% c("Full Retention", "Median Mortality")) %>% 
  mutate(status_equil = ifelse(n_div_k >= 0.5, "not overfished", "overfished")) %>% 
  group_by(status_equil) %>% 
  mutate(count = n()) %>% 
  left_join(full_pred, by = "scientific_name") %>% 
  drop_na(max_size_cm) %>% 
  distinct()

ggplot(data = sim_results_subset %>% 
         filter(max_size_cm < 800)) +
  geom_point(aes(x = max_size_cm, y = n_div_k, color = mort_scenario)) +
  ...
  annotate(geom = "text", label = "Not overfished", x = 700, y = 0.6, size = 4) +
  annotate(geom = "text", label = "Overfished", x = 700, y = 0.4, size = 4) +
  theme_bw()
```

...
prm_elasmo_subset <- prm_elasmo %>% 
  ...
  mutate(group = case_when(
    ...
    str_detect(scientific_name, "Himantura|Dasyatis|Gymnura|Neotrygon|Bathytoshia|rays") ~ "Batoids",
    TRUE ~ "Sharks"
  ))
...
# visualization of AVM proportions per species and family
avm_plot <-
...
mort_proportions_plot_fam <-
  ggplot(data = mort_summary_fam %>% 
           filter(!gear_class %in% c("handline", "purse seine", "pole and line")) %>% 
              mutate(estimate_type = str_to_title(estimate_type),
                     gear_class = str_to_title(gear_class))) +
  geom_line(aes(x = fct_rev(factor(family, levels = mort_subset_fam)), 
                 y = mortality_prop,
                 group = family),
            show.legend = F,
             color = "gray30",
             linewidth = 0.6,
             alpha = 0.5) +
  geom_point(aes(x = fct_rev(factor(family, levels = mort_subset_fam)), 
                 y = mortality_prop, 
                 color = estimate_type),
             show.legend = F,
             size = 1.5,
             alpha = 0.9) +
  facet_grid(factor(group, levels = c("Sharks", "Batoids")) ~ 
               factor(gear_class,
                    levels = c("Longline", "Gillnet", "Trawl")),
             scales = "free_y",
             space = "free_y") +
...
habitat_plot <- 
  ...
  geom_point(aes(x = habitat_associated, y = mortality_prop, 
                 color = estimate_type, group = estimate_type),
             position = position_jitterdodge(jitter.width = .1)) +
  geom_boxplot(aes(x = habitat_associated, y = mortality_prop, 
                   fill = estimate_type),
               alpha = 0.5) +
  facet_grid(factor(group, levels = c("Sharks", "Batoids")) ~
             factor(gear_class,
...
ggsave(mort_proportions_plot, file = paste0("mort_proportions_plot.png"), path = here::here("figs"), height = 14, width = 12)
```
...
ggplot(data = mort_summary %>% 
         filter(estimate_type != "post-release survival" & 
                  !gear_class %in% c("purse seine", "pole and line", "handline")) %>% 
         mutate(gear_class = str_to_title(gear_class))) +
  geom_jitter(aes(x = gear_class, y = )) +
  geom_boxplot(aes(x = gear_class, y = estimate),
               alpha = 0.7) +
  facet_grid(factor(group)~ estimate_type) +
  labs(x = "Gear class",
       y = "Proportion of mortality") +
  coord_flip() +
  theme_bw()

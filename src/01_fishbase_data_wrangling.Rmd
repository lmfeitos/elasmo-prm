...
shark_lm <- read_csv(here("data", "shark_spp_lm.csv")) %>% 
  ...
  ungroup() %>% 
  group_by(family) %>% 
  mutate(lm_new_fam = ifelse(is.na(mean_lm), mean(mean_lm, na.rm = TRUE), mean_lm)) %>% 
  mutate(mean_lm = ifelse(is.na(mean_lm), lm_new_gen, mean_lm),
         mean_lm = ifelse(is.nan(lm_new_gen), lm_new_fam, mean_lm)) %>% 
  select(scientific_name, family, group, mean_lm)
...
prm_elasmo <- read_csv(here("data", "prm_elasmo_subset.csv")) %>% 
  ...
  left_join(shark_lm, by = c("scientific_name", "family", "group")) 
...
new_dat <- read_csv(here::here("data", "predict_data.csv")) %>%
  ...
  left_join(shark_lm, by = c("scientific_name", "family"))  

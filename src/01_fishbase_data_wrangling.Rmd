...
```{r}
# get data from other FishBase tables
maturity_fb <- fb_tbl("maturity") %>% 
  select(Speccode, Lm) %>% 
  clean_names() %>% 
  rename(spec_code = speccode)
 
pop_qb <- fb_tbl("popqb") %>% 
  select(SpecCode, K, t0) %>% 
  clean_names()

# join these with the fishbase elasmo data
# test_fishbase_join <- test_fishbase %>% 
#   left_join(maturity_fb, by = "spec_code") %>% 
#   filter(group == "Sharks") %>% 
#   group_by(scientific_name) %>% 
#   mutate(mean_lm = mean(lm, na.rm = TRUE)) %>% 
#   distinct(spec_code, scientific_name, family, order, group, mean_lm) %>% 
#   ungroup() %>% 
#   mutate(species = scientific_name) %>% 
#   separate_wider_delim(species, names = c("genus", "species"), delim = " ") %>% 
#   group_by(genus) %>% 
#   mutate(lm_new_gen = ifelse(is.na(mean_lm), mean(mean_lm, na.rm = TRUE), mean_lm)) %>% 
#   mutate(lm_new_gen = round(lm_new_gen, 4)) %>% 
#   mutate(source = ifelse(is.na(mean_lm), NA, "FishBase"))
  # ungroup() %>% 
  # mutate(mean_lm = ifelse(is.na(mean_lm), lm_new_gen, mean_lm))

#write_csv(test_fishbase_join, file = here("data", "shark_spp_lm.csv"), na = "")

# read in the dataset of legnth at first maturity with data from external sources and recalculate averages
# shark_lm <- read_csv(here("data", "shark_spp_lm.csv")) %>% 
#   group_by(genus) %>% 
#   mutate(lm_new_gen = ifelse(is.na(mean_lm), mean(mean_lm, na.rm = TRUE), mean_lm)) %>% 
#   ungroup() %>% 
#   mutate(mean_lm = ifelse(is.na(mean_lm), lm_new_gen, mean_lm)) %>% 
#   mutate(family = ifelse(scientific_name == "Galeocerdo cuvier", "Carcharhinidae", family))
...
```{r}
# read in phylo distance data
shark_phylo_dist <- read_csv(here("data", "elasmo_phylo_dist.csv"))
...
prm_elasmo <- read_csv(here("data", "prm_elasmo_subset.csv")) %>% 
  ...
  left_join(shark_phylo_dist, by = c("scientific_name", "family", "group")) 
...
new_dat <- read_csv(here::here("data", "predict_data.csv")) %>%
  ...
  left_join(shark_phylo_dist, by = c("scientific_name", "family")) 
...
dat_new_mean_phylo = dat_new_mean %>% 
  filter(scientific_name %in% new_dat$scientific_name)

write_csv(dat_new_mean, here::here("data", "predict_data_phylo.csv"), col_names = TRUE, na = "")
```
...
# read in prm_hypoxia data
prm_hypoxia <- read_csv(here("data", "prm_hypoxia.csv")) %>% 
  mutate(source = "This study") %>% 
  select(scientific_name, family, genus, gear_class, estimate_type, estimate, sample_size, max_size_cm, measure, median_depth, reproductive_mode, group, ac, source)
...
avm_gilman_hypoxia <- bind_rows(gilman_join, prm_hypoxia) %>% 
  select(-c(record_uid, common_name))

write_csv(avm_gilman_hypoxia, file = here("data", "avm_gilman_hypoxia.csv"), na = "")
```

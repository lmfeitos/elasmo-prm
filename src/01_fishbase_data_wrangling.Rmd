...
dat = full_join(prm_elasmo, new_dat) %>%
  ...
  separate_wider_delim(species, names = c("genus", "species"), delim = " ", too_many = "debug") %>% 
  select(-species)
```
...
dat_new_mean <- dat %>%
  group_by(genus) %>% 
  mutate(eo_new1 = ifelse(is.na(eo), mean(eo, na.rm = TRUE), eo),
         ac_new1 = ifelse(is.na(ac), mean(ac, na.rm = TRUE), ac)) %>% 
  group_by(family) %>% 
  mutate(eo_new2 = ifelse(is.na(eo_new1), mean(eo_new1, na.rm = TRUE), eo),
         ac_new2 = ifelse(is.na(ac_new1), mean(ac_new1, na.rm = TRUE), ac)) %>% 
  group_by(order) %>% 
  mutate(eo_new = ifelse(is.na(eo_new2), mean(eo_new2, na.rm = TRUE), eo),
         ac_new = ifelse(is.na(ac_new2), mean(ac_new2, na.rm = TRUE), ac)) %>% 
  select(-eo_new2,-ac_new2, -eo_new1,-ac_new1)

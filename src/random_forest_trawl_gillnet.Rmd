...
trawl_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat_associated + reproductive_mode, data = elasmo_trawl) %>%
  step_normalize(all_numeric_predictors())

gillnet_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat_associated + reproductive_mode, data = elasmo_gillnet) %>%
  step_normalize(all_numeric_predictors())
```
...
```{r, include=FALSE}
trawl_fit <- fit(trawl_final, trawl_train) # fit the data to the training data
```

```{r, include=FALSE}
trawl_train_predict <- predict(object = trawl_fit, new_data = elasmo_trawl) %>% # predict the training set
  bind_cols(trawl_train) # bind training set column to prediction
```
...
```{r}
gill_train_metrics <- gill_train_predict %>%
  metrics(mortality_prop, .pred) # get testing data metrics
gill_train_metrics
```
...
```{r}
ggplot(trawl_quant_test, aes(x = mortality_prop, y = .pred_50)) + # plot ln of real versus ln of predicted
  geom_point() +
...
```{r}
ggplot(gillnet_train_predict, aes(x = mortality_prop, y = .pred)) + # plot ln of real versus ln of predicted
  geom_point() +
  geom_smooth(method = "loess") + # add trend line
  theme_bw() +
...
## Data distribution for the main five predictor variables for trawl 

```{r}
trawl_pred <- full_join(trawl_test_predict, trawl_train_predict)

ggplot(trawl_pred, aes(max_size_cm, .pred)) +
  geom_point() +
  geom_smooth(method = "loess") +
  theme_bw()

ggplot(trawl_pred, aes(median_depth, .pred)) +
  geom_point() +
  geom_smooth(method = "loess") +
  theme_bw()
...
ggplot(trawl_pred) +
  geom_boxplot(aes(habitat_associated, .pred)) +
  theme_bw()
```

## Data distribution for the main five predictor variables for gillnet

```{r}
gillnet_pred = gillnet_train_predict

ggplot(gillnet_pred, aes(max_size_cm, .pred)) +
  geom_point() +
  geom_smooth(method = "loess") +
  labs() +
  theme_bw()

ggplot(gillnet_pred, aes(median_depth, .pred)) +
  geom_point() +
  geom_smooth(method = "loess") +
  theme_bw()

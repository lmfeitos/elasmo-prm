...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, ac, measure, median_depth, reproductive_mode, family, eo
  ) %>%
...
elasmo_avm <- prm_elasmo_subset %>%
  ...
  select(ventilation_method, median_depth, max_size_cm, habitat, reproductive_mode, ac, mortality_prop, family) %>%
  distinct() %>%
  drop_na()
...
elasmo_prm <- prm_elasmo_subset %>%
  ...
  group_by(scientific_name, gear_class) %>%
  mutate(mortality_prop = weighted.mean(estimate, sample_size)) %>%
  select(ventilation_method, median_depth, max_size_cm, habitat, reproductive_mode, mortality_prop, ac, family) %>%
  distinct() %>%
  drop_na()
```
...
avm_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat + reproductive_mode + ac, data = avm_train) %>%
  step_normalize(all_numeric_predictors())

prm_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + reproductive_mode + ac, data = elasmo_prm) %>%
  step_normalize(all_numeric_predictors())
```
...
v1 <- avm_fit %>%
  ...
  scale_x_discrete(labels = c("Habitat", "Ventilation Method", "Reproductive Mode", "Maximum Size (cm)", "Median Depth (m)", "Hypoxia Vulnerability"))
v1
...
v2 <- prm_fit %>%
  ...
  scale_x_discrete(labels = c("Ventilation Method", "Median Depth", "Reproductive Mode", "Maximum Size (cm)", "Hypoxia Vulnerability"))
v2
```
...
full_predictions <- preds_bind(full_prediction_avm, prm_fit) %>%
  ...
  select(
    ...
    ac, avm_mort, family.x, family, prm_mort
  )

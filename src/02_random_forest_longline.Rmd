...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, reproductive_mode, family, ac, eo
  ) %>%
...
avm_recipe <- recipe(mortality_prop ~ reproductive_mode + median_depth + max_size_cm + habitat_associated + ventilation_method + ac + eo, data = avm_train) %>%
  step_normalize(all_numeric_predictors())

prm_recipe <- recipe(mortality_prop ~ reproductive_mode + max_size_cm + habitat_associated + ventilation_method + ac + eo, data = elasmo_prm) %>%
  step_normalize(all_numeric_predictors())
```

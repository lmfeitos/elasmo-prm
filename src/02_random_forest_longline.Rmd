...
avm_recipe <- recipe(mortality_prop ~ reproductive_mode + median_depth + max_size_cm + ac + eo + ventilation_method + habitat, data = avm_train) %>%
  step_normalize(all_numeric_predictors())

prm_recipe <- recipe(mortality_prop ~ reproductive_mode + max_size_cm + median_depth, data = elasmo_prm) %>%
  step_normalize(all_numeric_predictors())
```
...
v1 <- avm_fit %>%
  ...
  scale_x_discrete(labels = c("Ventilation Method", "Habitat", "Reproductive Mode", "Hypoxia Vulnerability", "Thermal Sensitivity", "Maximum Size (cm)", "Median Depth (m)"))
v1
...
v2 <- prm_fit %>%
  ...
  scale_x_discrete(labels = c("Reproductive Mode", "Median Depth", "Maximum Size (cm)"))
v2
```

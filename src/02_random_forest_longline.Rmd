...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, reproductive_mode, family, ac_new, eo_new
  ) %>%
...
avm_recipe <- recipe(mortality_prop ~ reproductive_mode + median_depth + max_size_cm + ac_new + eo_new, data = avm_train) %>%
...
v1 <- avm_fit %>%
  ...
  scale_x_discrete(labels = c("Reproductive Mode", "Hypoxia Vulnerability", "Thermal Sensitivity","Median Depth (m)", "Maximum Size (cm)"))
v1

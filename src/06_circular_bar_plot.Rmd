...
sim_results_iucn_pct_plot_m_ave <- sim_results_iucn_pct %>% 
  group_by(redlist_category) %>% 
  summarise(mean_n_div_k_rel_BAU = mean(n_div_k_diff_rel_BAU, na.rm = TRUE))
...
prop = ggplot(data = sim_results_iucn_pct_plot_m) +
  ...
  annotate("text",
    ...
    geom_col(aes(x = id, 
             ...
             show.legend = F,
           width = 0.8) +
  ylim(-40, max(sim_results_iucn_pct_plot_m$n_div_k_diff_rel_BAU)) +
 ...
 geom_text(data = label_data_m, 
            aes(x = id, y = n_div_k_diff_rel_BAU + 15, label = scientific_name, angle = angle),

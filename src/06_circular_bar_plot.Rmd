...
sim_results_iucn_pct_threat <- sim_results_iucn_pct_threat %>% 
  filter(redlist_category %in% c("VU", "CR", "EN"))
...
# get improvement averages per main fished family
diff_fam <- sim_results_iucn_pct %>% 
  group_by(family) %>% 
  summarise(mean_diff = mean(percent_diff))  
  filter(family %in% c("Lamnidae", "Sphyrnidae", "Carcharhinidae", "Alopiidae"))
```
...
ggsave(prop, file = paste0("fig4_test.pdf"), path = here::here("figs"), height = 20, width = 20)
```

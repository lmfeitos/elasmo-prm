...
trawl_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + reproductive_mode + habitat + ac, data = elasmo_trawl) %>%
  step_normalize(all_numeric_predictors())

gillnet_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + ac, data = elasmo_gillnet) %>%
  step_normalize(all_numeric_predictors())
```
...
#     grid = expand.grid(mtry = seq(1, 4, 1), trees = seq(100, 2000, 100), min_n = seq(1, 9, 2))
...
v1 <- trawl_fit %>%
  ...
  theme_bw(base_size = 14) +
  labs(y = "") +
  scale_x_discrete(labels = c("Hypoxia Tolerance", "Habitat", "Median Depth (m)", "Maximum Size (cm)", "Reproductive Mode"))
v1
...
v2 <- gillnet_fit %>%
  ...
  scale_x_discrete(labels = c("Hypoxia Tolerance", "Ventilation Method", "Maximum Size (cm)", "Median Depth (m)"))
v2
```

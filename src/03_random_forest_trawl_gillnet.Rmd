...
trawl_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + reproductive_mode + habitat_associated + ventilation_method, data = elasmo_trawl) %>%
  step_normalize(all_numeric_predictors())

gillnet_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + habitat_associated + ventilation_method, data = elasmo_gillnet) %>%
  step_normalize(all_numeric_predictors())
```
...
v1 <- trawl_fit %>%
  ...
  theme_bw(base_size = 14) +
  labs(y = "") +
  scale_x_discrete(labels = c("Ventilation Method", "Associated Habitat", "Reproductive Mode", "Median Depth (m)", "Maximum Size (cm)"))
v1
...
ggsave(plot1, file = paste0("trawl_gill_val.pdf"), path = here::here("figs", "supp"), height = 10, width = 15)
```

...
prm_elasmo <- read_csv(here("data", "prm_hypoxia.csv")) %>%
  ...
  mutate(ventilation_method = case_when(
    scientific_name == "Squalus acanthias" ~ "stationary",
...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, ac, reproductive_mode, family
  ) %>%
...
trawl_folds <- vfold_cv(elasmo_trawl, repeats = 5, v = 5)

gillnet_folds <- vfold_cv(elasmo_gillnet, repeats = 5, v = 5)
```
...
trawl_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + reproductive_mode + ac, data = elasmo_trawl) %>%
  step_normalize(all_numeric_predictors())

gillnet_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat + reproductive_mode + ac, data = elasmo_gillnet) %>%
  step_normalize(all_numeric_predictors())
```
...
Below displays the results of cross validation tuning for trawl AVM
...
Below displays the results of cross validation tuning for gillnets AVM
...
v1 <- trawl_fit %>%
  ...
  theme_bw(base_size = 14) +
  labs(y = "") +
  scale_x_discrete(labels = c("Reproductive Mode", "Hypoxia Tolerance", "Maximum Size (cm)", "Median Depth (m)"))
v1
...
v2 <- gillnet_fit %>%
  ...
  scale_x_discrete(labels = c("Habitat", "Ventilation Method", "Reproductive Mode", "Hypoxia Tolerance", "Maximum Size (cm)", "Median Depth (m)"))
v2
```

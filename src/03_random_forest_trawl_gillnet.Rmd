...
trawl_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm  + reproductive_mode + evol_dist + ac, data = elasmo_trawl) %>%
...
v1 <- trawl_fit %>%
  ...
  theme_bw(base_size = 14) +
  labs(y = "") +
  scale_x_discrete(labels = c("Associated Habitat", "Reproductive Mode", "Median Depth (m)", "Maximum Size (cm)"))
v1
...
v2 <- gillnet_fit %>%
  ...
  scale_x_discrete(labels = c("Associated Habitat", "Ventilation Method", "Median Depth (m)", "Maximum Size (cm)"))
v2
```

...
elasmo_trawl <- prm_elasmo_subset %>%
  ...
  select(ventilation_method, median_depth, max_size_cm, habitat, reproductive_mode, mortality_prop, family) %>%
  distinct() %>%
  drop_na()
...
trawl_folds <- vfold_cv(elasmo_trawl, repeats = 10, v = 3)

gillnet_folds <- vfold_cv(elasmo_gillnet, repeats = 10, v = 3)
```
...
#     grid = 10 # grid_regular(trees(), finalize(mtry(), elasmo_trawl), min_n(), levels = 5) %>% filter(mtry < 7)
...
#     grid = 10 # grid_regular(trees(), finalize(mtry(), elasmo_gillnet), min_n(), levels = 5) %>% filter(mtry < 7)
#   ) # use grid of parameters to test
```

...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, evol_dist, ac, reproductive_mode, family
  ) %>%
...
trawl_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + reproductive_mode + evol_dist + ac, data = elasmo_trawl) %>%
  step_normalize(all_numeric_predictors())

gillnet_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm +  reproductive_mode + evol_dist + ac, data = elasmo_gillnet) %>%
  step_normalize(all_numeric_predictors())
```
...
v1 <- trawl_fit %>%
  ...
  theme_bw(base_size = 14) +
  labs(y = "") +
  scale_x_discrete(labels = c("Reproductive Mode", "Hypoxia Tolerance", "Maximum Size (cm)",  "Evolutionary Distance", "Median Depth (m)"))
v1
...
v2 <- gillnet_fit %>%
  ...
  scale_x_discrete(labels = c("Ventilation Method", "Reproductive Mode", "Maximum Size (cm)", "Hypoxia Tolerance", "Evolutionary Distance", "Median Depth (m)"))
v2
```

...
p <- 
  ggplot() +
  geom_rect(data = no_cq, aes(xmin = -Inf, xmax = Inf, ymin = 1.1, ymax = 1.7, fill = as.factor(redlist_category))) +
...
p <- ggplot() +
  ...
  geom_rect(data = no_cq_sub,
            aes(xmin = -Inf, xmax = Inf, ymin = 1.05, ymax = 1.145, fill = as.factor(redlist_category))) +
  geom_line(data = no_cq_sub, aes(t, n_div_k, color = mort_scenario, group = total_mort), linewidth = 1.5) +
...
ggsave(p, file = paste0("species_full.pdf"), path = here::here("figs"), height = 10, width = 10)

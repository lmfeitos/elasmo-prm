...
trawl_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + reproductive_mode + max_size_cm + ac, data = elasmo_trawl) %>%
  step_normalize(all_numeric_predictors())

gillnet_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + reproductive_mode + max_size_cm + ac, data = elasmo_gillnet) %>%
  step_normalize(all_numeric_predictors())
```
...
gillnet_tune <- gillnet_wflw %>% # use cross validation to tune parameters
  tune_grid(
    ...
    grid = expand.grid(mtry = seq(1, 4, 1), trees = seq(100, 2000, 100), min_n = seq(1, 19, 5))
  ) # use grid of parameters to test
```

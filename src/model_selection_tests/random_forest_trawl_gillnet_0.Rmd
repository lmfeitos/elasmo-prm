...
trawl_tune <- trawl_wflw %>% # use cross validation to tune parameters
  tune_grid(
    ...
    grid = grid_regular(trees(), finalize(mtry(), elasmo_trawl), min_n(), levels = 10) %>% filter(mtry < 7)
...
gillnet_tune <- gillnet_wflw %>% # use cross validation to tune parameters
  tune_grid(
    ...
    grid = grid_regular(trees(), finalize(mtry(), elasmo_gillnet), min_n(), levels = 10) %>% filter(mtry < 7)
  ) # use grid of parameters to test
```

...
```{r, include=FALSE}
avm_fit_test3 <- fit(avm_final_test3, avm_train_test3) # fit the data to the training data
```

```{r, include=FALSE}
avm_train_predict_test3 <- predict(object = avm_fit_test3, new_data = avm_train_test3) %>% # predict the training set
  bind_cols(avm_train_test3) # bind training set column to prediction

avm_test_predict_test3 <- predict(object = avm_fit_test3, new_data = avm_test_test3) %>% # predict the training set
  bind_cols(avm_test_test3)# bind prediction to testing data column
```
...
```{r}
collect_metrics(avm_cv2_test3)

collect_metrics(prm_cv2_test3)
```

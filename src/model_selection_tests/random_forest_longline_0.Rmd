...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, reproductive_mode, family, ac, eo, evol_dist
  ) %>%
...
## At-vessel mortality
avm_split <- initial_split(elasmo_avm, prop = .70, strata = family)
...
avm_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat + reproductive_mode + ac + evol_dist, data = avm_train) %>%

...
## At-vessel mortality
avm_split <- initial_split(elasmo_avm, prop = .8, strata = family)
...
avm_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat + reproductive_mode + evol_dist + ac, data = avm_train) %>%
...
avm_tune <- avm_wflw %>% # use cross validation to tune parameters
  tune_grid(
    ...
    grid = grid_regular(trees(), finalize(mtry(), avm_train), min_n(), levels = 10) %>% filter(mtry < 7)
...
prm_tune <- prm_wflw %>% # use cross validation to tune parameters
  tune_grid(
    ...
    grid = grid_regular(trees(), finalize(mtry(), elasmo_prm), min_n(), levels = 10) %>% filter(mtry < 7)
  ) # use grid of parameters to test
```

...
```{r}
prm_elasmo <- read_csv(here("data", "avm_gilman_hypoxia.csv")) %>%
...
prm_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat + reproductive_mode, data = elasmo_prm) %>%
  step_normalize(all_numeric_predictors())
```

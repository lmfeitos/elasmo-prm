...
prm_elasmo <- read_csv(here("data", "prm_hypoxia.csv")) %>%
  ...
  dplyr::select(- c(family.x, family.y))
```
...
prm_elasmo_subset <- prm_elasmo %>%
  select(
    ...
    method, ventilation_method, max_size_cm, measure, median_depth, reproductive_mode, family, ac, eo, evol_dist, mean_lm
  ) %>%
  ...
  mutate(reproductive_mode_new = case_when(
    reproductive_mode == "placental viviparity" ~ "viviparous",
    str_detect(reproductive_mode,"yolk|histo|oophagy") ~ "ovoviviparous",
    TRUE ~ reproductive_mode
  )) %>% 
...
avm_recipe <- recipe(mortality_prop ~ ventilation_method + median_depth + max_size_cm + habitat + reproductive_mode + eo + ac + evol_dist + mean_lm, data = avm_train) %>%

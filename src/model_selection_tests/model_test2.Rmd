...
# Model without ventilation_mode

## Overview

This model aims to predict the likelihood of mortality per taxonomic family between sharks and rays using several species-specific bioecological features such as median temperature, median depth, reproductive method as well as technological features such as the fishing gear each species was captured with.

```{r}
set.seed(42)
# Create the recipes for each estimate
avm_recipe_test2 <- recipe(mortality_prop ~ gear_class + reproductive_mode + median_depth + max_size_cm + habitat_associated,
                     data = avm_train) %>% 
  step_normalize(all_numeric_predictors())

prm_recipe_test2 <- recipe(mortality_prop ~ gear_class + reproductive_mode + median_depth + max_size_cm + habitat_associated, 
                     data = elasmo_prm) %>% 
  step_normalize(all_numeric_predictors())
```
...
```{r}
# create models
avm_rf_test2 <- rand_forest(trees = tune(), mtry = tune(), min_n = tune(), mode = "regression") %>% 
  set_engine("ranger", verbose = TRUE, oob.error = TRUE, quantreg = TRUE, splitrule = "beta", importance = "impurity")

prm_rf_test2 <- rand_forest(trees = tune(), mtry = tune(), min_n = tune(), mode = "regression") %>% 
  set_engine("ranger", verbose = TRUE, oob.error = TRUE, quantreg = TRUE, splitrule = "beta", importance = "impurity")
```

```{r}
# create workflows
avm_wflw_test2 <- workflow() %>% 
  add_model(avm_rf) %>% 
  add_recipe(avm_recipe)

prm_wflw_test2 <- workflow() %>% 
  add_model(prm_rf) %>% 
...
```{r,include=FALSE}
avm_tune_test2 <- avm_wflw_test2 %>% # use cross validation to tune parameters
  tune_grid(
...
```{r,include=FALSE}
prm_tune_test2 <- prm_wflw_test2 %>% # use cross validation to tune parameters
  tune_grid(
...
```{r}
autoplot(avm_tune_test2) + # plot results of tuning
  theme_bw() # change theme
```

```{r}
select_best(avm_tune_test2, metric = "rsq", n = 1) # get the best model
```

```{r, include=FALSE}
avm_final_test2 <- finalize_workflow(avm_wflw_test2, select_best(avm_tune_test2, metric = "rsq")) # finalize the workflow with the best model
```

```{r, include=FALSE}
avm_fit_test2 <- fit(avm_final_test2, avm_train_test2) # fit the data to the training data
```

```{r, include=FALSE}
avm_train_predict_test2 <- predict(object = avm_fit_test2, new_data = avm_train_test2) %>% # predict the training set
  bind_cols(avm_train_test2) # bind training set column to prediction

avm_test_predict_test2 <- predict(object = avm_fit_test2, new_data = avm_test_test2) %>% # predict the training set
  bind_cols(avm_test_test2)# bind prediction to testing data column
```

```{r}
avm_train_metrics_test2 <- avm_train_predict_test2 %>%
  metrics(mortality_prop, .pred) # get testing data metrics
avm_train_metrics_test2

avm_test_metrics_test2 <- avm_test_predict_test2 %>%
  metrics(mortality_prop, .pred) # get testing data metrics
avm_test_metrics_test2
```

Below displays the results of cross validation tuning

```{r}
autoplot(prm_tune_test2) + # plot results of tuning
  theme_bw() # change theme
```

```{r}
select_best(prm_tune_test2, metric = "rsq", n = 1) # get the best model
```

```{r, include=FALSE}
prm_final_test2 <- finalize_workflow(prm_wflw_test2, select_best(prm_tune_test2, metric = "rsq")) # finalize the workflow with the best model
```

```{r, include=FALSE}
prm_fit_test2 <- fit(prm_final_test2, elasmo_prm) # fit the data to the training data
```

```{r, include=FALSE}
prm_train_predict_test2 <- predict(object = prm_fit_test2, new_data = elasmo_prm) %>% # predict the training set
...
```{r}
prm_train_metrics_test2 <- prm_train_predict_test2 %>%
  metrics(mortality_prop, .pred) # get testing data metrics
prm_train_metrics_test2
...
avm_cv2_test2 <- avm_final_test2 %>%
  fit_resamples(avm_folds, control = my_controls) # fit the folds to the final model

prm_cv2_test2 <- prm_final_test2 %>%
  fit_resamples(prm_folds, control = my_controls) # fit the folds to the final model
```

```{r}
avm_metrics_test2 <- collect_metrics(avm_cv2_test2) 
avm_metrics_test2

prm_metrics_test2 <- collect_metrics(prm_cv2_test2) 
prm_metrics_test2
```

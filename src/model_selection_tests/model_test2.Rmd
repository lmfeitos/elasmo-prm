...
avm_wflw_test2 <- workflow() %>% 
  add_model(avm_rf_test2) %>% 
  add_recipe(avm_recipe_test2)

prm_wflw_test2 <- workflow() %>% 
  add_model(prm_rf_test2) %>% 
  add_recipe(prm_recipe_test2)
```
...
```{r, include=FALSE}
avm_train_predict_test2 <- predict(object = avm_fit_test2, new_data = avm_train) %>% # predict the training set
  bind_cols(avm_train) # bind training set column to prediction

avm_test_predict_test2 <- predict(object = avm_fit_test2, new_data = avm_test) %>% # predict the training set

...
avm_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + reproductive_mode + ac, data = avm_train) %>%
  step_normalize(all_numeric_predictors())

prm_recipe <- recipe(mortality_prop ~ median_depth + max_size_cm + reproductive_mode + ac, data = elasmo_prm) %>%
  step_normalize(all_numeric_predictors())
```
...
avm_train_metrics_p <- avm_train_predict %>%
  filter(habitat == "pelagic") %>%
  metrics(mortality_prop, .pred) # get testing data metrics
avm_train_metrics_p

avm_train_metrics_d <- avm_train_predict %>%
...
avm_test_metrics_p <- avm_test_predict %>%
  filter(habitat == "pelagic") %>%
  metrics(mortality_prop, .pred) # get testing data metrics
avm_test_metrics_p

avm_test_metrics_d <- avm_test_predict %>%
  filter(habitat == "demersal") %>%
  metrics(mortality_prop, .pred) # get testing data metrics
avm_test_metrics_d
```

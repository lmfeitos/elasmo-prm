...
avm_recipe <- recipe(mortality_prop ~ median_depth + reproductive_mode + max_size_cm + ac, data = avm_train) %>%
...
avm_tune <- avm_wflw %>% # use cross validation to tune parameters
  tune_grid(
    ...
    grid = expand.grid(mtry = seq(1, 3, 1), trees = seq(100, 2000, 100), min_n = seq(1, 28, 5))
